...
server <- function(input, output, session) {
  ...
  extended_task <- ExtendedTask$new(
    function(x, y) mirai({Sys.sleep(y); runif(x)}, .args = list(x, y))
...
plotServer <- function(id, calc_time) {
  ...
  moduleServer(
    ...
    function(input, output, session) {
      extended_task <- ExtendedTask$new(
        function() mirai(run_task(calc_time), .args = list(run_task, calc_time))
      ) |> bind_task_button("resample")
      
      observeEvent(input$resample, extended_task$invoke())
...
```
3. In the server, create an observer on the input button, which invokes the 'ExtendedTask' object. In this case, no arguments are required as the parameters are already present in the evaluation context of the 'ExtendedTask'.
