...
server <- function(input, output, session) {
  ...
  extended_task <- ExtendedTask$new(
    function(x, y) mirai({Sys.sleep(y); runif(x)}, .args = environment())
...
2. In the server, create an 'ExtendedTask' object by calling `ExtendedTask$new()` on a function which is a call to `mirai()`, and optionally bind it to the button created in (1). Here, specifying `.args = environment()` conveniently passes the 'x' and 'y' variables from the calling environment (supplied by the anonymous function).
...
plotServer <- function(id, calc_time) {
  ...
  moduleServer(
    ...
    function(input, output, session) {
      extended_task <- ExtendedTask$new(
        function(x) mirai(run_task(calc_time = x), run_task = run_task, x = x)
