...
2. In the server, create an 'ExtendedTask' object by calling `ExtendedTask$new()` on a function passing '...' arguments to `mirai()`, and bind it to the button created in (1).
...
3. In the server, create an observer on the input button, which invokes the 'ExtendedTask' with the named parameters specified in the 'mirai' i.e. 'x' and 'y' in this example.

```{r shinystep3, eval=FALSE}
observeEvent(input$btn, extended_task$invoke(x = input$n, y = input$delay))
```

4. In the server, create a render function for the output, which consumes the 'ExtendedTask' result.
