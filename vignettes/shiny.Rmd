...
2. In the server, create an 'ExtendedTask' object by calling `ExtendedTask$new()` on a function which is a call to `mirai()`, and optionally bind it to the button created in (1). Here, passing in `environment()` conveniently captures the 'x' and 'y' variables from the calling environment (supplied by the anonymous function).
