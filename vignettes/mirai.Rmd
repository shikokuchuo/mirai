...
``` r
m[]
#> [1] 4.847675 4.733954 4.058473 4.013005 3.467577
```
...
m[]
#> [1] 3.253132 1.744983 2.881622 3.419026 3.701735
```
...
#> 
#> $daemons
#> [1] "abstract://4b234daa17931f3fdb383a5d"
#> 
#> $mirai
...
#> 
#> $daemons
#> [1] "abstract://6250622ac0b5451de270ba65"
```
...
#> [2] "  ..@ ptr                :<externalptr> "                           
#> [3] "  ..@ dbname             : chr \"/tmp/RtmpJhBcWA/file1a2f55d0376b\""
#> [4] "  ..@ loadable.extensions: logi TRUE"                               
#> [5] "  ..@ flags              : int 70"                                  
#> [6] "  ..@ vfs                : chr \"\""                                
#> [7] "  ..@ ref                :<environment: 0x6188d70daab0> "           
...
In this case, specify the 'url' for the `daemons()` call to be '127.0.0.1'. This is as the tunnel is created between `127.0.0.1:port` on each machine. The host listens to its `127.0.0.1:port` and the remotes each dial into `127.0.0.1:port` on their own respective machines.
...
launch_remote(2)
#> [1]
#> Rscript -e 'mirai::daemon("tcp://hostname:35785",dispatcher=TRUE,rs=c(10407,-316384744,415718713,1419414438,612319727,-231739548,1324271381))'
#> 
#> [2]
#> Rscript -e 'mirai::daemon("tcp://hostname:35785",dispatcher=TRUE,rs=c(10407,-2092710556,-680980252,-616313605,-978131980,2052481631,1937588957))'
daemons(0)
#> [1] 0
```
...
launch_remote(1)
#> [1]
#> Rscript -e 'mirai::daemon("tls+tcp://hostname:40025",dispatcher=TRUE,tls=c("-----BEGIN CERTIFICATE-----
...
#> ADCCAgoCggIBAMT8qq9nLUi5X2v1JjEGHyvgVRZIWEDTh+POr38x+yyqQI01Irn7
#> 0Z5qtEfsDY54OgTP7cvQrcNsa5Wm2x/z55lIrHZ7tgPdJ4fid4Sqf040StLUs9IL
#> sxSm+TPrnsUTunQFeAwaM7LyUW+HZLwyjJeUiRwryFmbwnMemdFmZ7wadQe3Xor0
#> u61BVul8uVM05sFpDL8q9HJTjXv6zJijvWEBVFBT8Xe2baSOpb1990vC7Cb9Inpm
#> T5HCZPAzWhKaOOzBFosbr2yM8mV5mK3t1/mFe1N8g7qZeqMRkE9O7pCWOFHeBkLq
#> 1m0XykbK+hEViyg6pFzVDPMElgd9nR8A6rb3dIS1qER61nbo61k9Rtf4/5yucp1h
#> su73AhzV3vCPg9yIXb6Ud4KOUuK0/sv9auym3g+pZB0lglhnDij1GXSkzc675nEh
#> 47ULZeetygpZetHiVkFnnY4s2hO39aYy0viIufKIyofQdVKJlGtYJHm+9qgeUYs+
#> w4nUhr7lJEejS+MxnCyJMpb38tkLupp0IUJLCjrGM4E1vJx5vDFgQb2070SMSEe9
#> FA7FKll9q8cqo/lGruOBf8iR42+yDibIjSchP0vc2r21izF7+nXLbDSRo7Ivhgks
#> YWcl1VYfh3Pe+YUZ5CHU9jPf1bx5ONUXEUTjaj1PbQRUJUZXwtWa1zPDAgMBAAGj
#> VjBUMBIGA1UdEwEB/wQIMAYBAf8CAQAwHQYDVR0OBBYEFJdLlDQXB6+bmtsr862T
#> CAZ6Bj2UMB8GA1UdIwQYMBaAFJdLlDQXB6+bmtsr862TCAZ6Bj2UMA0GCSqGSIb3
#> DQEBCwUAA4ICAQBSwDIHBzzShjcfGn01/Aefj9LWrQBSxvp9BeZs8FmtcV11ZSkf
#> RNevu6W8ytWSAwD4rcd/82P1S9rdxyZuHFfGv7cLDlYENVxVD66cxq3wTXDXSXqN
#> utaLHe55ScqcO6lKdHLwyAifjeglD5ifm4OP6y9ubPsugpwpskKROzCNYicPl/6s
#> sQsCJMOjSCsGlrtdlqAOiZqWA3qIZeKMLmwTwmrf3QIymcOovQSIbFLqTQz4n0SJ
#> H0hyKfU6qAvU3lEUi5stAGlkcpbMhObrFjpGwovbYl/sqni1QJaZhcZpUXNOOpmZ
#> F4otU9ptTeoP0DV3+NxCjCX25jTloRdHo8RFF1Sp90oJDV4G+JvmzuT+JifJumaC
#> oRwDDWAlzCQTFr50aXIAXildkrCpm1BDRXE5PgqkJN7KcVPFx8lEzuNWtv4zufSu
#> z5I0GjY/1Y36wf0sgiqxUiEgyRTcFytK1ye052x+Jg9hfT2rKtJ2kCmXN4r5zxp6
#> yU3JkzXM0FyczrqYVgDU4Vv9EWf5mW+PVWIP8gA2fNcRn97vlryPZTgXItN5aOHQ
#> 8s384Y4Jv9sJBrDjKtJ0896/lRMII2a2luaSyEJFInOTrJRUqQefgCoo0JBLKbgo
#> UKAYHp0bANz/x/PJWpGz+deCxupxMZWHVkyqWF01+QUJrSLja7sy1a4TjA==
#> -----END CERTIFICATE-----
#> ",""),rs=c(10407,410931870,1509437063,-727638756,-1868907667,1658429258,-727363517))'
```
