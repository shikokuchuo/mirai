...
``` r
m[]
#> [1] 3.090949 5.491539 4.761512 3.061476 3.913778
```
...
m[]
#> [1] 4.098618 3.684135 3.733703 5.165438 3.954965
```
...
#> 
#> $daemons
#> [1] "abstract://b5e12d724925b4c38fafcdd6"
#> 
#> $mirai
...
#> 
#> $daemons
#> [1] "abstract://08377b5769709d30e4390dc8"
```
...
#> [2] "  ..@ ptr                :<externalptr> "                           
#> [3] "  ..@ dbname             : chr \"/tmp/Rtmpdmaaal/file31371a96db13\""
#> [4] "  ..@ loadable.extensions: logi TRUE"                               
#> [5] "  ..@ flags              : int 70"                                  
#> [6] "  ..@ vfs                : chr \"\""                                
#> [7] "  ..@ ref                :<environment: 0x58e5dc2aff70> "           
...
To use tunnelling, supply a URL with hostname of '127.0.0.1' to 'url' for the `daemons()` call. `local_url(tcp = TRUE)` does this for you. For example, if 'local_url(tcp = TRUE, port = 5555)' is specified, the tunnel is created using port 5555 on each machine. The host listens to its `127.0.0.1:5555`, and the daemons each dial into `127.0.0.1:5555` on their own respective machines.
...
launch_remote(2)
#> [1]
#> Rscript -e 'mirai::daemon("tcp://hostname:44035",dispatcher=TRUE,rs=c(10407,667515964,-1813805043,-2094425238,-1063266845,868077000,-533139159))'
#> 
#> [2]
#> Rscript -e 'mirai::daemon("tcp://hostname:44035",dispatcher=TRUE,rs=c(10407,-530506672,2133572169,771037010,-1572233094,640780345,-588342701))'
daemons(0)
#> [1] 0
```
...
launch_remote(1)
#> [1]
#> Rscript -e 'mirai::daemon("tls+tcp://hostname:40659",dispatcher=TRUE,tls=c("-----BEGIN CERTIFICATE-----
...
#> ADCCAgoCggIBAK4cYjB9vwEwKH5sDyuQet6HPnFeuS9m6tC1tiuLW9CPfwrZ6b/u
#> xUG1/W9hazbVqScsa1Zrdoh2kNIprg32sJeCAiWBShLpwSHX5Ma9DOwa55OOaBZ9
#> AFBXIcYe8a2e5cRn9/ynID3uV5uw0jE97NO5blQEtBpUB3r1s6+C/mYub3ssjwp6
#> v4BPaaibliUPUoYpxTItLtYxufC/TMiVGEti5JzmAMxkyVzevzpzsbyYezPU8UqR
#> bhN1ZLQ8KPbDbTyehnV1/6c7VJFbOkE/QzU+CH/3emy/BNpSQrniYpFRg3VgSlrK
#> K9PjxwNpl+XbPxqPPoIpOA1YZ1+7uKWEIVN1ipIttw/POGlh3IsJDbnZ1apTVXGj
#> DBvAn9cB2HAiv6JKxgWGhhHoakAXrpWEi58xZR3e+rQw/4XIl/lMQnfF+oq93E94
#> zdjYeMn1+eCaN6W5HId0NNl+U0hgnI8t0Gp5NKFxKWFihCwCNRsiT+SmgqytweNo
#> f4JZBRv5FZvd5eR5DUPvRxc2F39FEu5v9hxVVUa94dptkc84uuwkorucUwq3VtEG
#> RFKkGZ1GuACgQElYTM/+Dicrb7WgHyo9G5pd5IFq0+MV/STLUtIg0SkUBIm4Pruq
#> DN2MMBRgee0HMvjhyIIwK+IkuV1aLyPAW2KBWwBFK/K5lDaHvXwKzFSdAgMBAAGj
#> VjBUMBIGA1UdEwEB/wQIMAYBAf8CAQAwHQYDVR0OBBYEFIQ8mOLbalGMkzh/vaq7
#> 0jW+w4W1MB8GA1UdIwQYMBaAFIQ8mOLbalGMkzh/vaq70jW+w4W1MA0GCSqGSIb3
#> DQEBCwUAA4ICAQCNnHCVNKHEtFgz3OQqB+t7EujBGU8qIwA6/5bsUY3424kN+D0V
#> 6G+lNukTp1R78Gt6bh6SkK4LuNl2UlQMPt2jYUjOTbektIghkt8TCB0kp0KKa0Cb
#> AWTku2tiQJgh+Q5+RQ0lXo8XsAcs/fL8LTYx2DrCnrdd1AG/1yicUsmqrqFkNXei
#> QX50zMrQ+ZYCHoZR7OBQ+MPajEwLEXTwNtO7bm7lr52fds4ntFyxzkVddFKLOPUN
#> qmZtk4txsvPglEJbmvoQqxAyZK6qOGq9K/laqY2Fo8Nvdxtfi3aF9bnaeEa2A052
#> I5RrLvRiZrFk2pwhS/tfd+2Ff9SwshmKj61kPq9OGZODBqX25ghizYQDx6q+wXKS
#> 7edOhH96E61Y1uA91SWsgFGmXU1atS/EU8dhYTSGLFaVjJ7FqvbslEqsf83i08l7
#> QGuW4e258k2uOo4XAQ2hgpCKfkIm3pOZ+oy+GmJL63AlGrgKN6LaDTx9VrKDWStA
#> mRmIijHyvMRwUH8qRGQwzgfVipcotAlQobWKAjMbh3nNkuTavittSTyZTTK8BNTe
#> 7f2GcBe4RfD2wWhlx07Ir5qnunk6jnB0owCvzGYsDnZwvSfF1go+RImjHxO6PK7C
#> Xt5SkO8mjulf0e7mmloDwNuzoseUls0XF06r78Xg3i75JCcWzux9JY5dmw==
#> -----END CERTIFICATE-----
#> ",""),rs=c(10407,-37763752,-1259438215,2129541350,-158515665,-646828380,460434773))'
```
