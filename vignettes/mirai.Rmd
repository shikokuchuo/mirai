...
```r
m$data |> str()
#>  num [1:100000000] -0.928 -0.823 -0.75 2.003 -1.447 ...
```
...
call_mirai(m)$data |> str()
#>  num [1:100000000] 5.539 -1.971 -0.856 2.657 2.169 ...
```
...
#> abstract://9f4d040e73047a54179a6587 1      1        1        0        0
#> abstract://32a6c8862ba67df99594cc8b 2      1        1        0        0
#> abstract://cb38db038e8ed24d1ac779d2 3      1        1        0        0
#> abstract://55c105673d93744b7a0605cc 4      1        1        0        0
#> abstract://24d5dfbeea2f3364978b9765 5      1        1        0        0
#> abstract://c34aa786916622a24069bf6d 6      1        1        0        0
```
...
#> 
#> $daemons
#> [1] "abstract://84f0a4362f0d75fb2e4c5984"
```
...
launch_remote(1:2)
#> [1]
#> Rscript -e "mirai::daemon('tcp://hostname:40403',rs=c(10407,-863499016,-1097663847,218460550,631617103,1178861636,-1195660683))"
#> 
#> [2]
#> Rscript -e "mirai::daemon('tcp://hostname:36069',rs=c(10407,-1439269584,-1960786011,2017930759,-383401367,1795121412,144529965))"
...
launch_remote(1)
#> [1]
#> Rscript -e "mirai::daemon('wss://hostname:33609/1',tls=c('-----BEGIN CERTIFICATE-----
...
#> ADCCAgoCggIBAN8SVta1edi8dk1zHEEF7drFphM9OIGsJNZEtHta07l75zRz7sMd
#> Eh13llJejU7cq2FH7QBsNDwNXH9KMYBVTQKpiZfdrgmtPCnMCfqjYIOXPftwMXtA
#> bXIHPEEZsJK88sSz34FuXiftiOoqy9vGsmZvUx9eByc9QIWp+beaqKKr25cVfAYH
#> dJkmganqvEAhJb+LvxYxUDNgjx2DfUfSq+WGpyDz1GnEeAzF4QwD3QUwkIIBVLDV
#> rdSUJ8xv+L0cxlb8Blu26YjMbkjHBsGBteKgCyyC1m+conioDyC3juAnoZFW4mt1
#> u70/xU0oFbho0neJUbR7k+WAOamQW3nBMOLdmzjlOQXeDZAsMwb6liU39xzHBft7
#> /mgOY1HUmiI6veI9LzlFkKqKre6BaIagZMgSLiN3LxrfV+PM4Dgwjxl97HgttpxD
#> InmM8uBoHIfJaO4sQsD4+RT49XrItJ06r/BiURDW3alh2qwKHt42RdITOkBT2weX
#> stp+CIDbM4/ThpHZ/xXYvgA2jOWteqntncRHbECjXIihwHafb+9ubGb/sw6CPvMT
#> lBtCmWpeY7hPVe3fdVcTeuZ2QnWF1tBU/a8C5EOEN3o5Z2nbElkbulWT/HdxbJqM
#> Z1/ASysNG1nOvUWn4WeHHOtctrUO0PzBHgE/KgqcL8EQhfENwHJYbfIzAgMBAAGj
#> VjBUMBIGA1UdEwEB/wQIMAYBAf8CAQAwHQYDVR0OBBYEFNZdUrobgvY9ejkt+tK+
#> lu6QKm2AMB8GA1UdIwQYMBaAFNZdUrobgvY9ejkt+tK+lu6QKm2AMA0GCSqGSIb3
#> DQEBCwUAA4ICAQCJXLb7nB91abXhVEysUP+NSXoC3wsVRHSJW2AmsTml8Z4+nnIv
#> 08T/OXOAYvePPzne2C0NC+cB3Jai9p9HnR0KwyKGHykMuUJvZ/OIj9KJbd8PgyNT
#> 5aoS6MfcrdBswvFZ7km612F/F/A6R2n22B0P8vWNs4AAnf3S7JO4gReth2BCPAXW
#> Fpf2xGKLB4RMtCldan1rR79iUCDbAT9mkyac+SKSmWW1+F1lpTiIX7sZ/W7sPQi1
#> OgrzppaphGn7yEpPmsv0f0qGiuEdTh8tcHY5LPDcKoCTnVCrqUoq7WC3vEgEtDD6
#> OV/3iXMrNFk341cJdC5C9Arf34Jy4JBBm3G0qBmItmg56/j8fnawJ1bW4Q9CK+F5
#> CbcctYsI41lWI/U8ESUfGi8Zd0LUAe8RPgbQZsZhFp0qJqjKU4/gawSTof4jEWCc
#> dSR2RHyGoIerB/hXVFOc68F3ohnTo+aErNzAPcc7La7FXKUuUA6ZLkUC+Qu3Z+iL
#> KIE4LiYx+/ogHIAIjvkoMCzo4rh9j4HsDHsFQf50/SILs6RqP1DNjgn0avOplHQq
#> vtJqSjwtMFdo/SgZrB6t3is9lFJGt9qD4qz4VL8kfkMrCmwoWLSEPJoyaQmx3rX9
#> GIgQEaejuXSf6KUH0uMXmNloOypGUai16ET5Jo/kTLPKXMx5pYUpC/S5Tw==
#> -----END CERTIFICATE-----
#> ',''),rs=c(10407,2055440879,1397069668,-825340139,1682859218,-1296603605,-751304848))"
```
...
Native R serialization is used for sending data between host and daemons. Some R objects by their nature cannot be serialized, such as those accessed via an external pointer. In these cases, performing 'mirai' operations on them will error.
...
x <- as_arrow_table(head(iris))

m <- mirai(list(a = as_arrow_table(x), b = "some text"), x = x)
call_mirai(m)$data
#> $a
#> Table
