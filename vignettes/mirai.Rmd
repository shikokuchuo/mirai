...
``` r
m$data |> str()
#>  num [1:100000000] 0.981 0.255 -0.609 0.54 -4.5 ...
```
...
call_mirai(m)$data |> str()
#>  num [1:100000000] 1.388 2.002 2.01 0.294 0.923 ...
```
...
#> abstract://541f1cd24ed2b38ea9ee2ef9 1      1        1        0        0
#> abstract://1ed30fe0da7954601b04c0fe 2      1        1        0        0
#> abstract://48441772847eab0966384e9f 3      1        1        0        0
#> abstract://7985d599953435309811f3eb 4      1        1        0        0
#> abstract://d0f42c54aec47d66b0b48be2 5      1        1        0        0
#> abstract://ca7bdda32fe57b128b0db16d 6      1        1        0        0
```
...
#> 
#> $daemons
#> [1] "abstract://614ce6b24b7a983e8a50b388"
```
...
#> [2] "  ..@ ptr                :<externalptr> "                           
#> [3] "  ..@ dbname             : chr \"/tmp/RtmpYan2UL/file8160280e39eb\""
#> [4] "  ..@ loadable.extensions: logi TRUE"                               
#> [5] "  ..@ flags              : int 70"                                  
#> [6] "  ..@ vfs                : chr \"\""                                
#> [7] "  ..@ ref                :<environment: 0x5b884baaf0b0> "           
...
launch_remote(1:2)
#> [1]
#> Rscript -e "mirai::daemon('tcp://hostname:43529',rs=c(10407,-1809384051,-1683606806,1201055587,-1584983736,1179745961,-245759402))"
#> 
#> [2]
#> Rscript -e "mirai::daemon('tcp://hostname:33729',rs=c(10407,623476174,-1373270933,-1689819727,-919487338,87106096,2135881594))"
```
...
launch_remote(1)
#> [1]
#> Rscript -e "mirai::daemon('wss://hostname:33845/1',tls=c('-----BEGIN CERTIFICATE-----
...
#> ADCCAgoCggIBALvsdgPN3o8SF1y1qVIU3SDdt8DZyNKmQWru8BtjQfcozBsa8ssk
#> JnUwGNAi1I671OZp6yDnafRo+SYorKf5DWZDfLpkS7yI57b8PJbSEkvRedZ+FuSk
#> GZqtZ5KoEhQatheTZgr0P7g9CUAHB6WuT+d1p8MLBYPkSO1a15pw+yaDF+hvLb5J
#> baxtKrITHfEFbZlpNQphj2fQcTwceBKHkbsph9qb4vjpbI1aatcLbsDX27MHKvKk
#> UFIwpeU8YvIPNFK5POYqLdyBqURXaagooE2k/PW+Ln8sLoPnkFOBrFib/Y5LxOZR
#> gqCnncig5FUV+t8pwhdhL6F4UPDKAzwP0F4nWMMpORIq6vquXFMMXIqXEVpoJcA8
#> 5LLMQlArgqy5DU5BTU0Pz377Dbp7q6PvdWWjWbk4blISDRH3/Vv6pPwBUSjm2kAC
#> N15rp2UqtCdVFgyzJ27vFE2+Id9KBi9aOoHosU5xacc8aV4XjJahocGKJSqKSN42
#> QN0wiIFfqTedsvnJbUNKLTtS02zqaPzflExskeOpP0BifoMQ5p6obZQ3zamiYXbw
#> 35lhXzRxd39vEerNnqYqEaRcvguC8OtnBK7fLhj650JxuuD0mqnNswQS3cHPEaKY
#> DM9rODO2Oaxa3id+RAmWVK0HtW/jhE1+Wk2ZF3g8/BkCJUyliJ1F4asBAgMBAAGj
#> VjBUMBIGA1UdEwEB/wQIMAYBAf8CAQAwHQYDVR0OBBYEFJldRzCgSZ3T4r3nIvPJ
#> MZlcdHWEMB8GA1UdIwQYMBaAFJldRzCgSZ3T4r3nIvPJMZlcdHWEMA0GCSqGSIb3
#> DQEBCwUAA4ICAQBRBEMNiYL4XhtAzOPx/0apXsFOWEPbIpG7QzgtFdxWv8YmT/a3
#> KK/yH6Bo/qdoykCAzX/3MbUMOru34/sj+/xapTaKrf06goIhFOQ3D/DoC8p99amv
#> uZTr6MBN329tQGyhhAoES9stfgp1k2FizngPOkGG0WZm6R5Dv2tCOzZQZWjYFxxb
#> Lg8jgj8MzvZq/E0lyoG63d6azI3mTuAZCrYxqTTlufr6/SktZeO3HFzzUFBZ1WrS
#> LDIyJZ2umPAWPalH9QYltAZxTmJtu7ZEtyk2eL3K7P5yE6WVq7Yquw5vv2vwWlVe
#> Qc+DywJp/A2RzlXRcaH1rFZeqUTuDI7CH2N8fdRP7BOkp4jdPBp4d4dLTOBpGI3q
#> hP4/dTtapzx0hDvWcoH1JhdzdMtVix4MBD2SJtKEytqpHSNoknGIML0jMajYcsnD
#> Vhc0+Z5eVoW/HWpWZ1/7aaBJfoJrCoSiKf6ioGUkSRQaFgEuZYIryM+TIZQE8sQa
#> rOmPEOWLaYGEZJVb1X82yiT2/QWBlTG0Js4QMrC7nUIIiqHQHn0X8xHDZnlbLgmy
#> 6LkC2p1dGPm+5hc2UeNTvN0ZWqmDgY4cHjG0enxrlbmdtXp4HLlSFCLivc5TWVm2
#> TTruR+m8oKC3+5NW1lff6c4xTDDaF/bvhtqwFxnwkJt2Ms0RWekCNVNiYA==
#> -----END CERTIFICATE-----
#> ',''),rs=c(10407,-1177468458,-1363175841,1080900884,1621617925,-628683262,-404860517))"
```
...
`mmap()`, short for 'mirai map', is analogous to `purrr:map()`.

`mwalk()`, short for 'mirai walk', is analogous to `purrr::walk()`, although completely asynchronous and returning a list of 'mirai' objects.

`mcollect()` can then be used to return the values from `mwalk()` or indeed any list of 'mirai' objects.
...
- Does not apply chunking (assigning multiple elements to daemons in batches), and hence suited to lengthy computations, or those with varying or unpredictable compute times over the indices. In these cases relying on `mirai` scheduling can be optimal.
...
#> 
#> [[3]]
#> [1] 18.94024 22.03892 23.19278
```
`mmap()` and `mwalk()` assume that `daemons()` have previously been set. Please be aware that if not, each computation will be run in a newly-created ephemeral daemon.
