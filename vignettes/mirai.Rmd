...
``` r
m[]
#> [1] 3.942299 4.734870 2.252801 4.822258 5.029710
```
...
m[]
#> [1] 4.285993 2.431517 5.693451 3.078171 2.425170
```
...
#> 
#> $daemons
#> [1] "abstract://8a28621b4649957c5a50857c"
#> 
#> $mirai
...
#> 
#> $daemons
#> [1] "abstract://053711fa47d356238360f0fd"
```
...
#> [2] "  ..@ ptr                :<externalptr> "                           
#> [3] "  ..@ dbname             : chr \"/tmp/Rtmpt7pdkd/filedb8751152173\""
#> [4] "  ..@ loadable.extensions: logi TRUE"                               
#> [5] "  ..@ flags              : int 70"                                  
#> [6] "  ..@ vfs                : chr \"\""                                
#> [7] "  ..@ ref                :<environment: 0x60c22819c400> "           
...
In these cases, SSH tunnelling offers a solution by creating a tunnel once the initial SSH connection is made. For simplicity, this SSH tunnelling implementation uses the same port on both the host and daemon. SSH key-based authentication must already be in place, but no other configuration is required.

To use tunnelling, specify the 'url' for the `daemons()` call as '127.0.0.1' with either a specific port or a port of '0' to use a free ephemeral port. For example, if 'tcp://127.0.0.1:5555' is specified, the tunnel is created using port 5555 on each machine. The host listens to its `127.0.0.1:5555`, and the remotes each dial into `127.0.0.1:5555` on their respective machines.
...
The below example launches 2 nodes on the remote machine 10.75.32.90 using SSH tunnelling ('url' hostname is specified as '127.0.0.1'):
...
launch_remote(2)
#> [1]
#> Rscript -e 'mirai::daemon("tcp://hostname:44913",dispatcher=TRUE,rs=c(10407,1458549102,2069998359,1845308524,459935357,1697051418,-308085293))'
#> 
#> [2]
#> Rscript -e 'mirai::daemon("tcp://hostname:44913",dispatcher=TRUE,rs=c(10407,-1015969887,2145712568,1333102764,1134273637,1284257239,1790437869))'
daemons(0)
#> [1] 0
```
...
launch_remote(1)
#> [1]
#> Rscript -e 'mirai::daemon("tls+tcp://hostname:34875",dispatcher=TRUE,tls=c("-----BEGIN CERTIFICATE-----
...
#> ADCCAgoCggIBALR2YKq6LHECbwLTCohtKAd/JAc5Q86x0vRswG4/hFOyLEQKnoIc
#> YOw3qf3MmN8bzsUPNP8PAW2NMK9JnH7HHjNPjL7xERK8grstjjJ8s6KSWQj9mDsd
#> 0sx8ynBHwppDva29gXlDwCMrV8xztxRuBa6AcrlN6EqKrKidnCHng8r7o3QqcwbQ
#> yDaSgglBTCAVVSGRGHUQfkx/UwXG+9B9AinAoZ3JCTGjvuOOnEEnBbSBf0lLSwYn
#> CAwMrqO5ujSRpsZtMIYffEibAF7kWu/nAK/8qRiR96ZG3JAkZSzg4apfTCEkzqyC
#> NpWL10lCoZDveBAWU6Il9qM8bZNY8bAZjjCmtqAqtKaq0Ljpr2hTVaLgEZAi3Pg0
#> l4Cqao1s6JydhELxU9aEydrVGbkYCSwcHCERe1/F47u3OBe23jTYkD94jjcEEEIH
#> B/mLUvKXv2ufa/Sidew6ukiol0Gkf+Y5Roxb4+QZamkqO5dbnM9a1L9dXguUiBYK
#> rwFvTUteAUNtPJ9CE52AYerYcU6MbwVefNTCcKsRI6SkPWyJKP4Xl12cX3ili+Xv
#> xNVv3tY4C/4d7XJrs589Bp0xJDtGo2G36BjECw/8Hpj5M67+N/5g2j2VkhsqgFdN
#> Yua06mDHPRqRkIeJ5ShNNqkOB/PMlF9KMoq21m+rBr2I15Q64reF5FBLAgMBAAGj
#> VjBUMBIGA1UdEwEB/wQIMAYBAf8CAQAwHQYDVR0OBBYEFDEMFU3BIxu37mjBk8RC
#> 9WC7gy0FMB8GA1UdIwQYMBaAFDEMFU3BIxu37mjBk8RC9WC7gy0FMA0GCSqGSIb3
#> DQEBCwUAA4ICAQBeBkNQGTJYN0UBwr61w4fd8ZI4J/FkqZ+SrTMOho8vwD3R8zEz
#> YQGz0fCJ2G8jUAZiuYHKHCUWzozPO9qkGAjFRnzwN9N1aDRrZvxxlY/AAn2enjvb
#> qSatSnPNMPBHL4BuewdvJWSMzwWHq5ak2GPKXcrDxlAFsTaueCLm+rydZ45e1JhA
#> E5YhGAxmtvrFu7wgzc/UIDXbMhjbQP3J6FoV/VsuyHa6m3wbCSxjjhY/TSHmBsIf
#> Ln+WgEvuGa+P+gKA4Okgn01uxc9eSCClgbHBmUz0ZgqepHtGHb2oKiIq7S75CxWF
#> cJYYQyTws+2OBMB6IQLTBPDQaAM3E/EwTMtIRq/sm9aCq5sutiGUqbfEONwoD+AY
#> f29M49LLKEpcudRkOgv0clJfiDNqHJqGw8ytgexcp9X0mDMWivsthi6GvsQtvQvW
#> ixIa+bLAaaQlKeQx1hw/WB2cecjinHN3K8j7iLo9lOKB3C4ool46i+ettYAA4n9R
#> sHmAPV5NioEUYQB9xFfix3dK9LI3nksb61xJOKqZ3TunUKCdcQVJf7DxYX9uwQP/
#> iaGnYMDhy9B4iQnfm70qmDGZliRLvHck+s9awpet+ijbtBuXZKeBlbvaEA6cAC20
#> gqzWywjiPU41iblsmxGVH24kFxBo4BYR6H3lNQka8JjI0qKcL8UygR+6IA==
#> -----END CERTIFICATE-----
#> ",""),rs=c(10407,605837864,-1250936311,937029174,-1194164417,809834612,1107720421))'
```
