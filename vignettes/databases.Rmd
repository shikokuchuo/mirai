...
The database connections are set up to begin with in all daemons using `everywhere()`, and the one-time `serialization()` setup ensures seamless transport of Apache Arrow data. Shiny ExtendedTask is then used to perform each request via a `mirai()` call.
...
ui <- page_fluid(
  ...
  p("Database connections hosted in 4 daemon processes"),
  sliderInput(
...
# use with() to automatically clean up resources when app exits
with(
  # use 4 local daemons
  daemons(4),
  {
    # create temp database file (for this demonstration)
    file <- tempfile()
    ...
    # write iris to the database (for this demonstration)
    m <- mirai(dbWriteTableArrow(con, "iris", iris))
    # run the Shiny app
    runApp(app)
    # disconnect from the database everywhere after app exits
    everywhere(dbDisconnect(con))
    # delete temp database file (for this demonstration)
    unlink(file)
  }
)
```
