...
It is possible using the `DBI` interface to access and manipulate data in the Apache Arrow data format.

The example below utilises the simplest in-memory database in a single background process.
...
Due to the tight integration of the `mirai` serialization mechanism with R's 'refhook' system, we can easily return complex / nested objects containing multiple queries comprising Arrow tables:
