...
if (.Platform[["OS.type"]] != "windows" || length(R.version$crt)) {
  ...
  nanotestz(daemons()[["daemons"]])
  Sys.sleep(0.3)
  nanotest(daemons(, value <- sprintf(mirai:::.urlfmt, nanonext::random()), dispatcher = FALSE) == value)
  nanotestz(daemons(0L))
  Sys.sleep(0.3)
  ...
  nanotestz(daemons(0L))
  Sys.sleep(1L)
  nanotesto(daemons(1L, dispatcher = FALSE, walltime = 500L, timerstart = 1L, .compute = "new"))
  ...
  if (Sys.getenv("NOT_CRAN") == "set") {
    nanotesto(daemons(1, dispatcher = TRUE, maxtasks = 10L))
    mq <- mirai("server")
