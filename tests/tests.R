...
nanotesterr(mirai(), "missing expression, perhaps wrap in {}?")
nanotesterr(mirai(a, 1), "all '...' arguments must be named")
nanotesterr(mirai(a, .args = list(1)), "all items in '.args' must be named")
...
nanotesterr(daemons(1, dispatcher = "p"), "must be one of")
nanotesterr(daemons(url = local_url(), dispatcher = "t"), "must be one of")
...
nanotesterr(everywhere({}), "not found")
...
connection && {
  ...
  if (is_mirai_error(me$data)) nanotest(is.list(me$data$stack.trace))
...
connection && .Platform[["OS.type"]] != "windows" && {
  ...
  nanotest(!is_mirai_map(m))
  nanotest(is.list(m))
  nanotest(length(m) == 3L)
  nanotest(all(as.logical(lapply(m, is.numeric))))
  Sys.sleep(1L)
...
connection && .Platform[["OS.type"]] != "windows" && Sys.getenv("NOT_CRAN") == "true" && {
  ...
  requireNamespace("promises", quietly = TRUE) && {
    ...
    nanotest(all(mp[.flat, .stop] == c(4L, 6L)))
  ...
  nanotest(tparse1[["port"]] != tparse2[["port"]])
  ...
  nanotest(is_error_value(m[]) || m[] == 2L)
  nanotestn(saisei(1))
