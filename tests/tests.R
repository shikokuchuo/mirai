...
test_one <- function(x) invisible(x == 1L || {print(x); stop("the above was returned instead of 1L")})
...
test_true(is.character(mlc <- launch_remote("ws://[::1]:5555")))
test_true(inherits(mlc, "miraiLaunchCmd"))
test_print(mlc)
...
test_error(launch_local(1L), "requires daemons to be set")
test_true(is.character(host_url()))
...
test_true(is.list(ssh_config("ssh://remotehost")))
test_true(is.list(ssh_config("ssh://remotehost", tunnel = TRUE, host = "tls+tcp://127.0.0.1:5555")))
...
connection && {
  ...
  test_true(is_error_value(m$data) || m$data == 3L)
  Sys.sleep(1L)
  `lang obj` <- quote(m + n + 2L)
  args <- c(m = 2L, n = 4L)
  m <- mirai(.expr = `lang obj`, .args = args, .timeout = 2000L)
  test_true(is_error_value(call_mirai_(m)$data) || m$data == 8L)
  test_null(stop_mirai(m))
  Sys.sleep(1L)
  test_one(d <- daemons(1L, dispatcher = FALSE, asyncdial = FALSE, seed = 1546L))
  test_print(d)
  ...
  test_true(is_error_value(dm$data) || is.matrix(dm$data))
  test_true(is.integer(status()[["connections"]]))
  test_true(is.character(status()[["daemons"]]))
  test_zero(daemons(0L))
  Sys.sleep(1L)
  test_one(daemons(1L, dispatcher = FALSE, idletime = 500L, timerstart = 1L, cleanup = FALSE, output = TRUE, .compute = "new"))
  test_true(is.character(nextget("urls", .compute = "new")))
  test_true(is.integer(nextstream(.compute = "new")))
  Sys.sleep(1.5)
  test_null(everywhere({}, as.environment(df), .compute = "new"))
  mn <- mirai("test1", .compute = "new")
  mp <- mirai(b + 1, .compute = "new")
  Sys.sleep(1L)
  test_true(unresolved(mn$data) || mn$data == "test1")
  test_true(unresolved(mp$data) || mp$data == 3)
  Sys.sleep(1L)
  test_true(is.integer(status(.compute = "new")[["connections"]]))
...
connection && .Platform[["OS.type"]] != "windows" && {
  ...
  test_zero(daemons(0L))
  Sys.sleep(1L)
  test_true(is.character(launch_remote("ws://[::1]:5555", remote = remote_config(command = "echo", args = list(c("Test out:", ".", ">/dev/null")), rscript = "/usr/lib/R/bin/Rscript"))))
  test_true(is.character(launch_remote("tcp://localhost:5555", remote = ssh_config(remotes = c("ssh://remotehost", "ssh://remotenode"), tunnel = TRUE, command = "echo"))))
  test_null(launch_local(local_url(), .compute = "test"))
  Sys.sleep(1L)
...
connection && {
  ...
  test_true(inherits(cluster, "miraiCluster"))
  test_true(inherits(cluster, "cluster"))
  test_equal(length(cluster), 1L)
  test_true(inherits(cluster[[1]], "miraiNode"))
  test_print(cluster[[1]])
  test_true(is.list(cluster[1]))
  test_true(is.character(launch_remote(cluster)))
  test_true(is.character(launch_remote(cluster[[1L]])))
  test_true(is.list(status(cluster)))
  ...
  test_true(is.list(res))
  test_equal(length(res), 10L)
  test_true(is.double(res[[1L]]))
  test_equal(length(res[[1L]]), 1L)
  test_true(is.double(res[[10L]]))
  ...
  Sys.sleep(1L)
  test_true(inherits(cl <- make_cluster(1), "miraiCluster"))
...
connection && .Platform[["OS.type"]] != "windows" && Sys.getenv("NOT_CRAN") == "true" && {
  Sys.sleep(1L)
  test_one(daemons(url = local_url(), dispatcher = "process", notused = "wrongtype"))
  ...
  requireNamespace("promises", quietly = TRUE) && {
    ...
    test_true(is.null(names(mp[])))
    test_true(is_error_value(mirai_map(1, function(x) stop(x), .promise = list(identity, identity))[][[1L]]))
  }
  Sys.sleep(1L)
  test_zero(daemons(NULL))
  test_one(daemons(url = "ws://:0", correctype = 0L, token = TRUE))
  test_zero(daemons(0L))
  test_zero(with(daemons(url = "tcp://:0", correcttype = c(1, 0), token = TRUE), {8L - 9L + 1L}))
  test_equal(daemons(n = 2, "ws://:0"), 2L)
  test_true(is.integer(nextget("pid")))
  test_equal(length(nextget("urls")), 2L)
  Sys.sleep(1L)
  status <- status()[["daemons"]]
  test_true(is.matrix(status))
  test_true(is.character(dn1 <- dimnames(status)[[1L]]))
  test_true(is.character(parse1 <- nanonext::parse_url(dn1[1L])))
  test_true(is.character(parse2 <- nanonext::parse_url(dn1[2L])))
  ...
  test_true(is.character(saisei(i = 1L)))
  test_null(saisei(i = 0L))
  test_true(is.character(saisei(i = 1L, force = TRUE)))
  test_null(saisei(i = 10L))
  test_zero(daemons(0))
  test_true(daemons(url = c("tcp://127.0.0.1:0", "tcp://127.0.0.1:0"), correcttype = NA) == 2L)
  Sys.sleep(1L)
  ...
  test_true(is.character(tdn1 <- dimnames(tstatus)[[1L]]))
  test_true(is.character(tparse1 <- nanonext::parse_url(tdn1[1L])))
  test_true(is.character(tparse2 <- nanonext::parse_url(tdn1[2L])))
  ...
  test_true(is.double(res <- mirai_map(c(1,1), rnorm)[.flat]))
  test_true(res[1L] != res[2L])
  test_zero(daemons(0))
  test_one(daemons(url = "wss://127.0.0.1:0", token = TRUE, pass = "test"))
  test_true(is.list(serialization(list(function(x) serialize(x, NULL), unserialize), "tst_cls")))
  ...
  test_true(is_error_value(m[]) || m[] == 2L)
  test_null(saisei(1))
  ...
  test_one(daemons(1, dispatcher = TRUE, maxtasks = 10L, timerstart = 1L, walltime = 1000L, seed = 1546, token = TRUE, cleanup = option, autoexit = tools::SIGCONT))
