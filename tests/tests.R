...
nanotest(is.list(daemons_view()))
nanotest(daemons_view()[["daemons"]] == 0L)
nanotest(daemons_view()[["connections"]] == 0L)
n <- 3L
m <- mirai({
  Sys.sleep(0.1)
  q <- m + n + 1L
...
nanotest(daemons_view()[["daemons"]] == 1L)
nanotest(daemons(0L) == 0L)
nanotest(daemons_view()[["daemons"]] == 0L)
nanotest(daemons(sprintf(mirai:::.urlfmt, runif(1, 1000000, 9999999))) == 1L)
nanotest(daemons(sprintf(mirai:::.urlfmt, runif(1, 1000000, 9999999))) == 1L)
...
nanotesterr(daemons(FALSE), "numeric or character")
