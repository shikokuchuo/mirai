...
connection && {
  ...
  test_error(launch_remote(remote = ssh_config(remotes = "ssh://127.0.0.1:5456", tunnel = TRUE)), "127.0.0.1")
  test_zero(daemons(0L))
  Sys.sleep(1L)
...
connection && Sys.getenv("NOT_CRAN") == "true" && {
  ...
  test_type("character", launch_remote(remote = ssh_config(remotes = c("ssh://remotehost", "ssh://remotenode"), tunnel = TRUE, command = "echo")))
