...
Sys.sleep(2.2)
nanotest(daemons(1L, active = FALSE) == 1L)
me <- mirai(mirai())
...
nanotest(daemons()[["daemons"]] == 0L)
Sys.sleep(0.2)
nanotest(daemons(url = value <- sprintf(mirai:::.urlfmt, runif(1, 1000000, 9999999)), active = FALSE) == value)
Sys.sleep(0.2)
nanotest(daemons(url = value <- "ws://:0", active = FALSE) != value)
nanotest(daemons(0L) == 0L)
Sys.sleep(0.2)
nanotest(daemons(1L, active = FALSE, walltime = 500L, timerstart = 1L, .compute = "new") == 1L)
...
if (Sys.getenv("NOT_CRAN") == "set") {
  nanotest(daemons(1, active = TRUE, maxtasks = 10L) == 1L)
  mq <- mirai("server")
