...
connection && .Platform[["OS.type"]] != "windows" && Sys.getenv("NOT_CRAN") == "true" && {
  ...
  option <- 15L
  cfg <- serial_config("cusom", function(x) serialize(x, NULL), unserialize)
  Sys.setenv(R_DEFAULT_PACKAGES = "stats,utils")
  nanotesto(daemons(1, dispatcher = TRUE, serial = cfg, maxtasks = 10L, timerstart = 1L, walltime = 1000L, seed = 1546, token = TRUE, cleanup = option, autoexit = tools::SIGCONT))
