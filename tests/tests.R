...
nanotestn(everywhere(mirai::register()))
...
nanotest(is_error_value(call_mirai(m)$data) || m$data == 8L)
Sys.sleep(2.5)
...
nanotest(mp$data == 3 || unresolved(mp$data))
Sys.sleep(1L)
...
if (Sys.getenv("NOT_CRAN") == "true" && .Platform[["OS.type"]] != "windows") {
  ...
  nanotest(is.pairlist(register(function(x) serialize(x, NULL), unserialize)))
  nanotest(is.function(register()[[1L]]))
  ...
  nanotestz(daemons(0L))
  nanotestn(register(NULL, NULL)[[2L]])
  Sys.sleep(1L)
  option <- 15L
