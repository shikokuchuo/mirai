...
nanotesterr(launch_remote(c("tcp://localhost:5555", "tcp://localhost:6666", "tcp://localhost:7777"), command = "echo", args = list(c("test", "."), c("test", "."))), "length of 'args'")
nanotesterr(launch_local(1L), "requires daemons to be set")
nanotesto(daemons(1L, dispatcher = FALSE, resilience = FALSE))
...
Sys.sleep(1L)
nanotest(is.list(ssh_args("ssh://remotehost")))
nanotesterr(ssh_args("ssh://remotehost", tunnel = TRUE), "must be called in the correct context")
if (.Platform[["OS.type"]] != "windows") {
  ...
  nanotest(is.character(launch_remote("tcp://localhost:5555", command = "echo", args = ssh_args(c("ssh://remotehost", "ssh://remotenode"), tunnel = TRUE))))
  Sys.sleep(2.5)
