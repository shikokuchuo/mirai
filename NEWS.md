# mirai 0.9.0.9012 (development)

* Enables secure TLS connections for distributed computing. Simply specify a `tls+tcp://` or `wss://` URL in `daemons()` and an automatic one-time configuration will be generated by default.
* New `cpinfo()` utility for retrieving information for a specific compute profile (number of daemons set, dispatcher process ID, daemon URLs, and generated TLS configurations).
* Requires nanonext >= [0.9.0.9027].
