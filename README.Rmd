...
The default implementation is low-level and ensures tasks are evenly-distributed amongst daemons without actively managing a task queue. This robust and resource-light approach is particularly well-suited to working with similar-length tasks, or where the number of concurrent tasks typically does not exceed the number of available daemons.

Alternatively, specify `q = TRUE` to maintain an active queue (task scheduler). This consumes additional resources, however ensures optimal allocation of tasks to daemons such that they are run as soon as resources become available.
...
On the server side, the `server()` function may be called from an R session, or Rscript from a suitable shell, to set up a remote daemon process that connects to the client network IP address ('192.168.0.2' in the example below):
...
Alternatively, use the `serverq()` function to allocate an active queue over a cluster of daemon processes on the server instead of a single instance.
