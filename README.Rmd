...
The generated self-signed certificate is made available for read-only access through the `info()` function specifying 'alt = TRUE', as shown below:
...
The returned character vector may be passed directly to the 'tls' argument of `daemon()` for launching daemons on remote resources.

`launch()` with the argument 'exec = FALSE' may also be used to conveniently generate the full shell command to launch a `daemon()`. This may be directly deployed on a remote machine, for example via a resource manager.
