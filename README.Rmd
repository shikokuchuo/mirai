...
{mirai} has a tiny pure R code base, relying solely on {nanonext}, a high-performance binding for the 'NNG' C library with no further package dependencies.
...
Through the `daemons()` interface, tasks may also be sent to server processes on the network for computation, rather than on the local machine.

Simply set the '.url' argument to the client network address (or leave blank to listen on all interfaces on the host) and a port that is able to accept incoming connections, for example:
...
On the server, run something like the following from a suitable shell to set up a remote daemon process, where '192.168.0.2' is the network IP address of the client:
