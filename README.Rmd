...
Specify as a character string the client network address and a port that is able to accept incoming connections, or use '0.0.0.0' to listen on all interfaces on the host, for example:
...
On the server, `server()` or `serverq()` may be called from an R session, or Rscript from a suitable shell, to set up a remote daemon process, or cluster of processes, that connects to the client network IP address ('192.168.0.2' in the example below):
