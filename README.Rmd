...
`instance` increments by 1 every time there is a new connection, regardless of whether the URL has been regenerated by `saisei()`. When this happens, the `assigned` and `complete` statistics will also reset. This is designed to track new server instances connecting after previous ones have ended (due to time-outs etc.).
