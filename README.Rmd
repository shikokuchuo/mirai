...
Alternatively, specifying `dispatcher = FALSE` invokes immediate dispatch. 
...
Requesting the status now shows 6 connections and 6 daemons. The background processes connect directly into the client and the number of daemons and connections are the same.
...
When using a dispatcher, there is only a single connection to the local background dispatcher process, which connects in turn to the servers running on remote resources.
