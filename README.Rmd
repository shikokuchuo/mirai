...
All that is required is to specify a secure URL of the form `wss://` or `tls+tcp://`. For example:
...
Alternatively, the key/certificate pair may be specified as the 'tls' argument to `daemons()`, and the public key certificate chain as the 'tls' argument to `server()`. The private key is retained on the client, and only the public key certificates need to be distributed to the server daemons.
