...
<br /><br />
Designed for simplicity, a 'mirai' evaluates an R expression asynchronously in a separate process, on the local machine or over the network, returning the result automatically upon completion.
<br /><br />
...
<br /><br />
Advantages include being inherently queued, allowing the sending of many more tasks than available connections, no arbitrary connection limits, no storage on the filesystem, support for otherwise non-exportable reference objects, an event-driven 'promises' implementation, and very low overhead. A parallel map function is included.
...
```
Above, the variables `time` and `mean` in the mirai expression are defined by `name = value` pairs passed through via the `...` argument.

A 'mirai' object is returned immediately. Whilst the async operation is ongoing, attempting to access the data yields an 'unresolved' logical NA.
```{r do}
m
m$data
```
...
`mirai_map()` maps a function over a list or vector, with each element processed in parallel in a daemon process.
...
```
A 'mirai_map' object is returned immediately, and its value may be retrieved using its `[]` method to return a list, just like `purrr::map()` or `base::lapply()`.
```{r mapvalue}
m
m[]
```
`mirai_map()` is designed to facilitate recovery from partial failure, and also provides options for early stopping and/or progress indicators.
