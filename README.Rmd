...
To view the current status, `info()` provides the number of active connections along with a matrix of statistics for each daemon.
...
Note that above, the port number is specified as zero. This is a wildcard value that will automatically cause a free ephemeral port to be assigned. The actual assigned port is provided as the return value of the call, or it may be queried at any time by via `info()`.
...
The generated self-signed certificate is stored as a weak reference, allowing read-only access (but not modification) through the `info()` function as shown below:
...
This character vector may be passed directly to the 'tls' argument of `daemon()` for launching daemons on remote resources.

`bquote()` may also be used to conveniently generate calls to `daemon()`, as in the example below. The resulting language object may be further deparsed to a character vector, as required, for deployment on the remote machine.
...
Similarly requesting information using `info()`, or launching daemons using `launch()` should specify the desired '.compute' argument.
