...
The package revolves around one single function:
 ...
 - `eval_mirai()` to send an expression for async evaluation, returning a 'mirai' which resolves automatically
...
The calculation is still ongoing hence an 'unresolved' logical NA value is returned initially.
...
When it completes, the 'mirai' will automatically return the evaluated result when queried.
...
Use `unresolved()` to poll for completion on a periodic basis, whilst continuing to perform tasks.
...
Peform other dependent tasks after confirming a successful write, such as performing the next write etc.
