...
<br /><br />
Advantages include being inherently queued (handling many more tasks than available processes), no storage on the file system, support for otherwise non-exportable reference objects, an event-driven 'promises' implementation, built-in asynchronous parallel map functionality, and very low overhead.
...
[Joe Cheng](https://github.com/jcheng5/) for optimising the `promises` method to work seamlessly within Shiny, and prototyping event-driven promises, which is implemented across `nanonext` and `mirai`.
