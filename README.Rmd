...
`mirai` also serves as the backend for enterprise asynchronous [`shiny`](https://shiny.rstudio.com/) applications. The [`crew`](https://wlandau.github.io/crew/) package provides a Shiny vignette with tutorial and sample code for this purpose. The [`crew`](https://wlandau.github.io/crew/) interface provides a nice abstraction layer that makes it easy to deploy `mirai` for [`shiny`](https://shiny.rstudio.com/); `mirai` itself is sufficient, although requires the individual `mirai()` requests to be managed using a list or equivalent.
