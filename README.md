...
``` r
m$data |> str()
#>  num [1:100000000] 0.626 -0.342 5.688 1.138 -0.343 ...
```
...
call_mirai(m)$data |> str()
#>  num [1:100000000] 0.925 2 0.498 0.47 2.234 ...
```
...
#> abstract://a6e5c6e0a134cbaac61ef4399d6e693e73d4903f      1        1        0        0
#> abstract://0353f524b7a1cf42dd3214e7f01210cf9765cd22      1        1        0        0
#> abstract://96c0bef1230e5647012acd74fbfe204081f54b3f      1        1        0        0
#> abstract://d02aea0cd4e32663f933a216962d101dcf55707f      1        1        0        0
#> abstract://e80b2f9f965ef8ca039a14549ed02d9db2e0e0dd      1        1        0        0
#> abstract://575d0d9a1baa796c488dbdc98ade7eef03b47e0b      1        1        0        0
```
...
`instance` increments by 1 every time there is a new connection,
regardless of whether the URL has been regenerated by `saisei()`. When
this happens, the `assigned` and `complete` statistics will also reset.
This is designed to track new server instances connecting after previous
ones have ended (due to time-outs etc.).
...
connects to the client URL and receives tasks:
    ...
    Rscript -e 'mirai::server("tcp://10.111.5.13:41245")'
