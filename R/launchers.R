...
#'
#'   **or** for `launch_remote` only, a \sQuote{miraiCluster} or
#'   \sQuote{miraiNode}.
...
#'   \sQuote{idletime} and \sQuote{walltime}. Only supply to override arguments
#'   originally provided to [daemons()], otherwise those will be used instead.
...
#'   classed as \sQuote{miraiLaunchCmd}. The printed output may be copy / pasted
...
#'   machine as a character string (e.g. `'ssh'`). Defaults to \sQuote{ssh}
#'   for `ssh_config`, although may be substituted for the full path to a
#'   specific SSH application. The default NULL for `remote_config` does not
#'   effect any launches, but causes [launch_remote()] to return the shell
#'   commands for manual deployment on remote machines.
...
#'   daemon launch command (not required for Slurm \sQuote{srun} for example,
#'   but required for \sQuote{ssh} or Slurm \sQuote{sbatch}).
#'
...
#'   If TRUE, requires the [daemons()] `url` hostname to be '127.0.0.1'. See the
...
#' \sQuote{127.0.0.1}. Using [local_url()] with `tcp = TRUE` also does this for
#' you. Specifying a specific port to use is optional, with a random ephemeral
#' port assigned otherwise. For example, specifying 'tcp://127.0.0.1:5555' uses
#' the local port '5555' to create the tunnel on each machine. The host listens
