...
#'   at which to listen for remote daemons to dial into, including a port
#'   accepting incoming connections (and optionally for websockets, a path),
#'   e.g. 'tcp://hostname:5555' or 'ws://10.75.32.70:5555/path'. Specify a URL
...
#'   runs in a separate process to ensure optimal scheduling, although this may
#'   not always be required (for details see Dispatcher section below).
...
#' if supplied.
#'
#' It is recommended to use a TCP URL such as \sQuote{tcp://10.75.32.70:5555}.
...
#' allows a unique path after the port number, which may support sharing the
...
#'
#' Switching the URL scheme to \sQuote{tls+tcp://} or \sQuote{wss://}
#' automatically upgrades the connection to use TLS. The auxiliary function
#' \code{\link{host_url}} may be used to construct a valid host URL based on the
#' computer's hostname.
#'
